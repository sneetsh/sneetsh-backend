...
const commonTemplate = ({ content, subject }) =>
  `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{font-family:Arial,sans-serif;padding:20px}@media screen and (max-width: 480px) {.logo {max-width: 80px;}}.footer{margin-top:2em;background-color:#252729;color:#fff;text-align:center;padding:10px;font-size:small;font-family:Arial,Helvetica,sans-serif}.footer p{line-height:12px}.footer img{margin:1.5em auto}.logo{max-width:100px;height:auto;display:block;margin:0 auto}</style></head><body><h3>${subject}</h3> ${content}</body></html>`;
