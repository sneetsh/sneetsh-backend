...
import TOKEN_TYPES from '../../../common/constants/token.constant';
import { MoreThan, Repository } from 'typeorm';
import { InjectRepository } from '@nestjs/typeorm';
import { User } from '../../user/entities/user.entity';
import { Token } from '../../user/entities/token.entity';
import { SendMailDTO } from '../../../common/validations';
import { generateRandomNumbers } from '../../../common/helpers';
import {
  excludeProps,
  generateLoginJWT,
  getMoment,
...
import { EmailTypes } from '../../../common/services/email-templates/auth.template';
...
export class AuthService {
  ...
  async signup(payload: UserSignupDto) {
    ...
    if (user)
      throw new ConflictException(
        'User with details already exist, please login',
      );
  ...
  async login({ identity, password }) {
    ...
    if (!user)
      throw new UnauthorizedException('Identity or password incorrect');
  ...
  async activate(payload: ActivateDTO) {
    ...
    user = user.toJson();
    const { refresh_token, access_token } = await generateLoginJWT(user);
  ...
  async accessTokenRefresh(payload: AccessTokenRefreshDTO) {
    ...
    const token = await this.tokenRepository
      ...
      .where('tokens.token =:refresh_token AND tokens.identifier =:user_id', {
        refresh_token,
        user_id,
      })
      .getOne();
    ...
    if (!token)
      throw new UnauthorizedException(
        'Invalid token, please login is required',
      );
